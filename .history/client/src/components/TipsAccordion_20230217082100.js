    ...
    return (
      <Accordion key={ `tip-${tip.id}` } expanded={expanded === `panel${tip.id}`} onChange={handleChange(`panel${tip.id}`)}>
        <AccordionSummary aria-controls={`panel${tip.id}d-content`} id={`panel${tip.id}d-header`}>
          ...
          <Box display="flex" justifyContent="flex-end "alignItems="flex-end"></Box><Avatar alt={ avatarSrc } src={ avatarSrc } sx={{ width: 24, height: 24 }}/>
        </AccordionSummary>
        <AccordionDetails>
          <Typography>
          { tip.content }
          ...
          { console.log("Tip User ID: " + tip.user_id)}
